...
class MouseOutput(NamedTuple):
    ...
    insert_logits: Optional[torch.FloatTensor]
    """
    A tensor of shape `(batch_size, seq_len, insert_vocab_size)` representing the log probabilities
    for the next insert token *before* normalization via (log) softmax.
    """

    time_logit: Optional[torch.FloatTensor]
...
class MouseModel(nn.Module):
            ...
            self.transformer.update(
                {
                    ...
                    "insert_ff_out": nn.Linear(
                        config.d_model,
                        1,
                        bias=config.include_bias,
                        device=config.init_device,
                    ),
                    "time_logit": nn.Linear(
                        config.d_model,
                        1,
                        bias=config.include_bias,
                        device=config.init_device,
    ...
    def forward(
        ...
        return MouseOutput(logits=logits, attn_key_values=attn_key_values, hidden_states=tuple(all_hidden_states) if output_hidden_states else None)  # type: ignore[arg-type]
