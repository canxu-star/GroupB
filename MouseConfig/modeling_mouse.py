...
from dataclasses import fields, dataclass # 新增 dataclass 导入
...
# [替换] modeling_mouse.py 中的 RotaryEmbedding 类

class RotaryEmbedding(nn.Module):
    """
    [Rotary positional embeddings (RoPE)](https://arxiv.org/abs/2104.09864).
    
    MODIFIED: This implementation is modified to accept an optional `pos_idx` tensor
    to handle non-sequential or gapped positional encoding.
    """
    ...
    def __init__(self, config: ModelConfig, cache: BufferCache):
        ...
        # RoPE is applied to a fraction of the hidden size per head.
        dim = config.d_model // config.n_heads
        
        # Pre-calculate inverse frequencies (theta_i) and register as a buffer
        # This part is static and can be pre-computed.
    ...
    def apply_rotary_pos_emb(self, pos_sin: torch.Tensor, pos_cos: torch.Tensor, t: torch.Tensor) -> torch.Tensor:
        ...
        # t: (batch, n_heads, seq_len, head_dim)
        # pos_cos/sin: (batch, 1, seq_len, head_dim) or (1, 1, seq_len, head_dim)
    ...
    def forward(
        ...
        Args:
            q, k: Tensors of shape (batch_size, n_heads, seq_len, head_dim).
            pos_idx: An optional tensor of shape (batch_size, seq_len) with values 0 or 1.
                     If provided, RoPE positions are calculated as the cumulative sum of `pos_idx`.
                     If None, standard sequential positions are used.
        """
        # We compute embeddings on the fly, so we use the input tensor's device.
        device = q.device
        seq_len = q.shape[-2]

        # Determine the positions for RoPE calculation
        if pos_idx is not None:
            # `pos_idx` is (batch, seq_len). Calculate cumulative sum to get positions.
            # Example: pos_idx = [1, 1, 0, 1] -> positions = [0, 1, 1, 2]
            # The token at index 2 (where pos_idx=0) gets RoPE for pos 1, but will be ignored later if needed.
            # A better way: cumulative sum and then subtract 1.
            positions = (torch.cumsum(pos_idx.float(), dim=1) - 1).long()
            # Ensure positions are non-negative
            positions.clamp_(min=0)
            
            # freqs shape: (batch_size, seq_len, dim / 2)
            freqs = torch.einsum("bs,d->bsd", positions.float().to(device), self.inv_freq.to(device))
            
        else:
            # Fallback to default sequential behavior if pos_idx is not provided.
        ...
        # Both branches produce `freqs`, now create sin/cos embeddings
        emb = torch.cat((freqs, freqs), dim=-1) # (..., dim)
        
        # Add a head dimension for broadcasting: (batch, 1, seq_len, dim)
        pos_sin = emb.sin().unsqueeze(1)
        pos_cos = emb.cos().unsqueeze(1)

        # Apply precision settings
        if self.config.rope_full_precision:
            q_, k_ = q.float(), k.float()
            pos_sin = pos_sin.float()
            pos_cos = pos_cos.float()
        else:
            q_, k_ = q, k
...
# =========================================================================================
class SequenceAggregationHead(nn.Module):
    """
    一个聚合 token 级隐藏状态为序列级表示，并输出单个值的头。
    这里使用平均池化作为示例。您也可以轻松改为使用最后一个 token 的状态等。
    """
    def __init__(self, config: ModelConfig):
        super().__init__()
        self.config = config
        # 定义一个从 d_model 映射到 1 个输出单元的线性层
        self.predictor = nn.Linear(config.d_model, 1, bias=config.include_bias, device=config.init_device)

    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        # 聚合策略：对序列长度维度（dim=1）进行平均池化
        # hidden_states 的形状: (batch_size, seq_len, d_model)
        pooled_output = hidden_states.mean(dim=1)
        # pooled_output 的形状: (batch_size, d_model)

        # 传入线性层得到最终输出
        # output 的形状: (batch_size, 1)
        output = self.predictor(pooled_output)

        # 去掉多余的维度，返回形状为 (batch_size,) 的张量
...
class MouseBlock(nn.Module):
    ...
    def attention(
        ...
        pos_idx: Optional[torch.LongTensor] = None,  # <-- 新增
        ...
        if self.config.rope:
            ...
            q, k = self.rotary_emb(q, k, pos_idx=pos_idx) # <-- 修改
    ...
    @classmethod
    def build(cls, layer_id: int, config: ModelConfig, cache: BufferCache) -> MouseBlock:
        if config.block_type == BlockType.sequential:
            return MouseSequentialBlock(layer_id, config, cache)
        elif config.block_type == BlockType.llama:
            return MouseLlamaBlock(layer_id, config, cache)
        else:
            raise NotImplementedError(f"Unknown block type: '{config.block_type}'")


class MouseSequentialBlock(MouseBlock):
    """
    ...
    def forward(
        ...
        if self._activation_checkpoint_fn is not None:
            ...
            att, cache = self.attention(q, k, v, attention_bias, layer_past=layer_past, use_cache=use_cache, pos_idx=pos_idx) # <-- 修改
...
class MouseLlamaBlock(MouseBlock):
    """
    This is a transformer block where the output is computed as ``MLP(LN(x + Attention(LN(x))))``
    (plus another skip connection). This block is similar to `MouseSequentialBlock`
    ...
    def forward(
        ...
        if self._activation_checkpoint_fn is not None:
            ...
            att, cache = self.attention(q, k, v, attention_bias, layer_past=layer_past, use_cache=use_cache, pos_idx=pos_idx)
...
# ===== 修改开始：定义新的输出结构 =====
class MouseOutput(NamedTuple):
    """
    修改 MouseOutput 以包含三个输出头的 logits
    """
    logits: torch.FloatTensor
    """
    A tensor of shape `(batch_size, seq_len, vocab_size)` representing the log probabilities
    for the next token *before* normalization via (log) softmax for the main head.
    """
    
    insert_logits: torch.FloatTensor
    """
...
class MouseGenerateOutput(NamedTuple):
...
class MouseBlockGroup(nn.ModuleList):
    ...
    def forward(
        ...
        for block_idx, block in enumerate(self):
            ...
            if (
                ...
                # shape: (batch_size, seq_len, d_model)
                x, cache = block(x, attention_bias=attention_bias, layer_past=layer_past, use_cache=use_cache, pos_idx=pos_idx) # <-- 修改
...
class MouseModel(nn.Module):
    def __init__(self, config: ModelConfig, init_params: bool = True):
        ...
        blocks = [MouseBlock.build(i, config, self.__cache) for i in range(config.n_layers)]
        if self.config.block_group_size > 1:
            block_groups = [
                MouseBlockGroup(config, i, blocks[i : i + config.block_group_size])
        ...
        act_fn_for_head2 = Activation.build(config)
        
        # 第二个输出头的网络层
        self.transformer.update({
            "ff_proj_head2": nn.Linear(
                config.d_model, self.hidden_size, bias=config.include_bias, device=config.init_device
            ),
            "up_proj_head2": nn.Linear(
                config.d_model, self.hidden_size, bias=config.include_bias, device=config.init_device
            ),
...
class MouseModelLM(PreTrainedModel):
    """
    ...
    def __init__(self, config: MouseConfig, model: Optional[MouseModel] = None, init_params: bool = False):
        ...
        if not model:
            ...
            self.model = MouseModel(model_config, init_params=init_params)
        else:
            self.model = model
    ...
    def forward(
    ...
    ) -> Union[Tuple, MouseOutput]: # ===== 修改返回类型 =====
        if use_cache is None:
            use_cache = self.config.use_cache

        if output_attentions:
            raise ValueError("output_attentions is not yet supported in Mouse")
        ...
        if labels is not None:
            import warnings
            warnings.warn("Note that for Mouse, you cannot calculate the loss here.", UserWarning)
            # 注意: 如果您需要计算损失，您需要决定使用哪个头的输出来计算，
            # 例如:
    ...
    """
    TODO:There are OCT B's operations
    """
